...
def all_ind(stock):
    open, close, high, low, volume = stock['open'], stock['close'], stock['high'], stock['low'], stock['volume']
    return ochl2buy(open, close, high, low, volume)
...
def ochl2buy(open, close, high, low, volume):
    ...
    data['pct'] = PCT_CHANGE(close) * 100
    # 当天涨幅
    data['pct_center'] = (close - open) * 100 / close_ref
    # 当天涨跌
    data['updown'] = -1
    data['updown'] = data['updown'].where(data['pct_center'] < 0, 1)
    # 下引线占比 当天涨为正，当天跌为负
    data['pct_down'] = (close.where(close < open, open) - low) * data['updown'] * 100 / close_ref
    # 上引线占比 当天涨为正 当天跌为负
    data['pct_up'] = (high - close.where(close > open, open)) * data['updown'] * 100 / close_ref
    # 当天最大振幅 当天涨为正 当天跌为负
    data['pct_all'] = (high - low) * data['updown'] * 100 / close_ref
    # 今开较昨收的涨幅
    data['pct_open'] = (open - close_ref) * 100 / close_ref
    ...
    sma_20 = SMA(close, 20)
    sma_60 = SMA(close, 60)
    data['close_sma20_pct'] = (close - sma_20) * 100 / close_ref
    data['close_sma60_pct'] = (close - sma_60) * 100 / close_ref
    data['sma20_sma60_pct'] = (sma_20 - sma_60) * 100 / close_ref

    diff, dea, hist = MACD(close, 12, 26, 9)
    data['diff_pct'] = PCT_CHANGE(diff) * 100
    data['dea_pct'] = PCT_CHANGE(dea) * 100
    data['diff_dea_pct'] = (diff - dea) / REF(dea) * 100

    data['diff_down_cross_0'] = DOWN_CROSS(diff, 0)
    data['diff_up_cross_0'] = UP_CROSS(diff, 0)

    data['dea_down_cross_0'] = DOWN_CROSS(dea, 0)
    data['dea_up_cross_0'] = UP_CROSS(dea, 0)

    data['diff_dea_golden_cross'] = GOLDEN_CROSS(diff, dea)
    data['diff_dea_dead_cross'] = DEAD_CROSS(diff, dea)
