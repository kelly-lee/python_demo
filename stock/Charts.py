...
def drawSMA(ax, price, periods=[5, 10, 20, 30, 60, 120]):
    for period in periods:
        ...
        ax.plot(sma, label='sma%d' % period)
...
def drawWR(ax, prices, periods=[14], hlines=[-80, -20]):
    high, low, close = prices[0], prices[1], prices[2]
    for period in periods:
...
def drawCCI(ax, prices, periods=[6, 12], hlines=[-100, 100]):
    high, low, close = prices[0], prices[1], prices[2]
    for period in periods:
...
def drawDMI(ax, prices, periods=[14, 6], hlines=[10, 20, 40]):
    high, low, close = prices[0], prices[1], prices[2]
...
date, high, low, open, close, volume = data['Date'], data['High'], data['Low'], data['Open'], data['Close'], data['Volume']
...
mpf.candlestick_ohlc(ax, zip(date_index, open, high, low, close), width=0.6,
